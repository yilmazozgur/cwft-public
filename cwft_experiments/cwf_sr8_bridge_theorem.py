...
def main():
    ...
    theorem = (
        "THEOREM (bridge (b), under H1+H2) -- PROVEN at the algebraic level, conditional "
        "as stated. The i in Renou's sigma_Y measurement is not an independent imaginary "
        "unit: (1)-(2) it is the action of the complex structure J0 = sigma_Z sigma_X "
        "(J0^2=-I, real antisymmetric), with sigma_Y = -i J0 = i[sigma_X,sigma_Z]/2; (3) "
        "J0 and sigma_Y share the eigenstates |y+->; (4) UNIQUENESS -- on the real 2-plane "
        "this complex structure is unique up to sign, so there is exactly ONE i; (5) sr1 "
        "[H1] forces precisely this structure (self-negation is a reflection with no real "
        "continuous realization; sqrt(NOT) eigenvalue i), and by (4) it is J0; (6) Renou's "
        "separation requires sigma_Y (removing it drops T below the real bound). Chaining: "
        ...
        "Hence the separation's complex resource IS the self-referential complex structure. "
        "Bridge (b) is a theorem under (H1) sr1 and (H2) the standard single-complex-"
        "structure quantum consistency. RESIDUAL CONJECTURE, flagged not closed: that "
