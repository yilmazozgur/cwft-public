...
def main():
    ...
    ax.axis("off")
    table_text = (
        "PHASE 1 (HP recovery):    All 5 generic substrates achieve full recovery $I(R,L)=2k$ near $|L|/N \\approx 0.5$, matching the\n"
        "                          Page-time prediction. Confirms information IS recoverable from radiation -- ENCODING-BASED HORIZON.\n\n"
        "PHASE 2 (Page curves):    All 5 generic substrates produce textbook Page curves $S(L) = \\min(L, N-L)$ when fully scrambled.\n"
        ...
        "PHASE 4 (scrambling t*): Power-law alpha=2 essentially N-independent, all-to-all flat-to-weak (fast scramblers); 2D brick-wall\n"
        "                          grows weakly; MERA-tree ~log N only per tree pass (sqrt(N)-like in elementary gate layers).\n\n"
        ...
        "    that the chapter's Einstein-dynamics condition / remaining-route conjecture identifies as the only candidate for\n"
        "    computational Einstein dynamics."
    )
