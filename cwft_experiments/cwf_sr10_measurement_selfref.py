...
def main():
    ...
    theorem = (
        "THEOREM (B) -- a complete, faithful, reversible measurement on a CLOSED substrate "
        "necessarily invokes the self-referential i; PROVEN modulo the isolated premise of "
        "closure. Chain (each link verified): (P-closure) the apparatus is part of the "
        "substrate, so a complete measurement is the substrate recording a part of itself; "
        "(1) by C11 a faithful COMMUTING self-record is impossible (no unitary clones "
        "non-commuting self-observables, <0|+>!=<0|+>^2), so the record must carry "
        "components anti-commuting with the pointer ({Z,X}=0); (2) recording such a "
        "component makes the pointer record the NEGATION of itself -- the measurement Liar "
        "P=NOT(P), with no definite outcome; (3) by sr1 the unique faithful + reversible "
        "realization is sqrt(NOT) (eigenvalue i), and it is genuinely complex -- det(NOT)=-1 "
        "forbids any real reversible realization; (4) by sr8 that i is J0=i*sigma_Y, exactly "
